...
- On a MacBook with a notch, the top edge takes the notch's own shape: straight sides, the hardware's rounding, moulded into the frame with a small fillet rather than flaring, at least as wide as the hardware, with the readings sitting below the hole. Folded away it *is* the notch — nothing shows at rest — and reaching for it makes the notch grow. Electron does not know about the camera housing, so AppKit is asked through the scripting bridge and the answer is remembered per display. It is on by default and can be turned off in Settings → Appearance → Placement (shown for the top edge), which keeps the top dock as its own shape, free to drag along the edge. Setting `CAPSULE_FAKE_NOTCH=200x37` while developing pretends the display has a notch, for trying the joined look on a Mac without one.
