...
- The Electron / Capsule icon no longer comes back in the macOS Dock after launch. Hiding the tile with `app.dock.hide()` is ignored if it happens twice in a second, and opening Settings or onboarding promoted the process back to a regular app. Capsule now runs as an accessory (menu-bar) app and stays that way when a window is focused.
